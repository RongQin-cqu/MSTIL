...
'''
model = EfficientNet.from_pretrained('efficientnet-b3', advprop=True, num_classes=3).cuda()
...
for epoch in range(1):
    ...
    for i, (images, labels) in enumerate(train_loader2):
        ...
        outputs,x= model(images)   #前向传播求出预测的值
...
for index, parma in enumerate(model.classifier.parameters()):
     if index == 6:
        parma.requires_grad = True
'''
model._fc=nn.Linear(1536, 13).cuda()
    ...
    for i, (images, labels) in enumerate(train_loader):
        ...
        if (i+1) % step_size == 0:
            print('Epoch [%d/%d], Iter [%d/%d] Loss: %.4f'
                    % (epoch, num_epochs, (i+1) / step_size, len(train_Data) / (batch_size * step_size), running_loss / step_size))
            running_loss = 0.0
